...
class gui_dark(Tk.Frame):
    ...
    def __init__(self,parent):
        ...
        #input guess file
        gfile = open('test_parameters.txt','r')
        ...
        #After months of the model overestimating, a new parameter was added
        #Effective June 2021 a new offset parameter (Lower) was added to imporve alignment
        self.fix2021 = 2.48e+08
...
#set the information based on screen size
        ...
        y =  (self.parent.winfo_screenheight())*0.5
        
        #y =  (self.parent.winfo_screenheight())*2.0
...
#Create the figure
        self.f,self.a = plt.subplots(ncols=2,figsize=(12*aratio,2*aratio*.75),sharex=True)
...
#set up print, refit, and quit buttons
        ...
        #quitButton.pack(side=Tk.RIGHT,padx=.1,pady=1)
        printButton = Tk.Button(self,text="Print",command=self.Print)
        printButton.pack(side=Tk.RIGHT,padx=.1,pady=1)
        refitButton = Tk.Button(self,text="Refit",command=self.refit)
        refitButton.pack(side=Tk.RIGHT,padx=.1,pady=1)
        #resetButton = Tk.Button(self,text="Reset",command=self.reset)
        #resetButton.pack(side=Tk.RIGHT,padx=.1,pady=1)
        ...
        inp_lab_per.pack(side=Tk.RIGHT,padx=0.1,pady=1)


        inp_val_per = Tk.StringVar(value='{0:10}'.format(np.inf))
        self.val_per = Tk.Entry(self,textvariable=inp_val_per,width=1)
        self.val_per.bind("<Return>",self.set_limt_param)
        self.val_per.pack(side=Tk.RIGHT,padx=1,pady=1)
        ...
        for i in self.b_keys:
            ...
            self.check_box[i].pack(side=Tk.LEFT,padx=0,pady=0)

        #set up box showing which parameters to freeze
        inp_lab_per = Tk.Label(self,textvariable=Tk.StringVar(value='Freeze Checked Par. = '),height=1,width=2)
    ...
    #format name of file to read
        ...
        ##############################################
        '''print(self.fdata['fuv3'][0])
        print(self.fdata['fuv3'][1][-24:-12])
        print(self.fdata['fuv3'][1][-12:])
        
        
        self.fdata['fuv3'][1].setflags(write=1)
        avrg = (self.fdata['fuv4'][1][-5] + self.fdata['fuv4'][1][-7])/2
        self.fdata['fuv4'][1][-6] = avrg
        self.fdata['fuv4'][1].setflags(write=0)'''
...
################################################################################################################
#               OUTDATED AND NOT USED, SEE BELOW FOR CURRENT MODEL
################################################################################################################
        c = 2.*np.pi
...
################################################################################################################
    ...
    def offset(self,dt0,amp1,amp2,p1,phi1,phi2,trend,quad,off,qscale,bo_drop,sc_amp,ns_incr, lower):
        ...
        ################################################################################################################
        #Incorporate new LOWER parameter starting in June 2021 (self.fix2021  = 2.48e+08)
        #For each port, the parameter drops the model down

        fix2021 = [(dt0 > self.fix2021)]
        

        #Subtract the new parameter for each port to provide better alignment
        #Only applies to the model following June 2021

        #de_trend_amplit = ((amp1*np.sin(c*(dt0/p1+phi1)))+(amp2*np.sin(c*(dt0/(p1/2.)+phi2))))*lower
        #trend[newcycle] = (trend+de_trend_amplit)[newcycle]
    ...
    ##########################################################################
    def Print(self):
        print('      {0:10},{1:10},{2:15},{3:10},{4:10},{5:20},{6:20},{7:10},{8:10},{9:10},{10:10},{11:10},{11:20}'.format('Amp1','Amp2','P1','Phi1','Phi2','Trend','Quad','Offset','Scale','OffDrop','AmpInc','OffNSop','Lower'))
        for i in self.b_keys:
