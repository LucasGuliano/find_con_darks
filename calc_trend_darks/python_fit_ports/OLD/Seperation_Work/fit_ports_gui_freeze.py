...
###################  FIRST SET OF PARAMETERS ############################
        ...
        ogfile = open('initial_parameters_2018.txt','r')
        ...
        for i,line in enumerate(ogfile):
            ...
            if i == 0: self.plis=line.split(',')
                ...
                self.ogdict[sline[0]] = [float(j) for j in sline[1].split(',')]
...
###################    CURRENT PARAMETERS    #####################
        #input guess file
        gfile = open('initial_parameters_postfrozen.txt','r')
...
#######################################################################
        ...
        #datetime.fromtimestamp(284014800).strftime("%m/%d/%Y %I:%M")
        ...
        #After months of the model overestimating, a new parameter was added (LRG)
        #Effective June 2021 a new offset parameter (Lower) was added to imporve alignment
        self.fix2021 = 1.33865e+09
        
        
        #Parameter for model freeze testing
        #self.freeze_before = 1.31e+09
        self.freeze_before = 1.27e+09
        ################################################
...
################################################################################################################       
    ...
    def offset(self,dt0,amp1,amp2,p1,phi1,phi2,trend,quad,off,qscale,bo_drop,sc_amp,ns_incr,lower):
        """
        ...
        lower: float
            Term to improve alignment after consistent overestimate seen starting June 2021. 
        ...
        #Define the OriGinal parameters for earlier frozen model
        port = (self.cport)
        og_amp1 = self.ogdict[port][0]
        og_amp2 = self.ogdict[port][1]
        og_p1 = self.ogdict[port][2]
        og_phi1 = self.ogdict[port][3]
        og_phi2 = self.ogdict[port][4]
        og_trend = self.ogdict[port][5]
        og_quad = self.ogdict[port][6]
        og_off = self.ogdict[port][7]
        og_qscale = self.ogdict[port][8]
        og_bo_drop = self.ogdict[port][9]
        og_sc_amp = self.ogdict[port][10]
        og_ns_incr = self.ogdict[port][11]
        ...
        og_off = np.zeros(dtq.size)+og_off
        #adjust offset for given quad flattening time
        og_off[adj] = og_off[adj]+og_quad*(1-og_qscale)*(self.dtq1[self.ptype]-self.dtq0[self.ptype])**2+og_trend*(1-og_qscale)*(self.dtq1[self.ptype])
        ...
        ################################################################################################################
        #Default config
        trend = (og_amp1*np.sin(c*(dt0/og_p1+og_phi1)))+(og_amp2*np.sin(c*(dt0/(og_p1/2.)+og_phi2)))+(og_trend*(dtl))+(og_quad*(dtq**2.00))+(og_off)
        ################################################################################################################
        ...
        drop_trend_offset = -(og_bo_drop)*og_off
        
        #increase the amplitutude of the periodic terms after the June 2018 bake out by a fractional amount
        incs_trend_amplit = ((og_amp1*np.sin(c*(dt0/og_p1+og_phi1)))+(og_amp2*np.sin(c*(dt0/(og_p1/2.)+og_phi2))))*og_sc_amp
        ...
        #NEW NEW NEW NEW
        ################################################################################################################
        post_testy = [(dt0 > self.freeze_before - self.t0dict[self.cport])]
        
        new_trend = (amp1*np.sin(c*(dt0/p1+phi1)))+(amp2*np.sin(c*(dt0/(p1/2.)+phi2)))+(off)
 
        ################################################################################################################
        #Incorporate new LOWER parameter starting in June 2021 (self.fix2021  = 2.48e+08 + start times in t0dict)
        #fix2021 = [(dt0 > self.fix2021 - self.t0dict[self.cport])]
        
        #Subtract the new parameter for each port to provide better alignment
        #Only applies to the model following June 2021
        #trend[fix2021] = (trend-lower)[fix2021]
        ################################################################################################################
        
        trend[post_testy] = new_trend[post_testy]
...
##################################################################################################################    
    ...
    ##########################################################################
    def Print(self):
        print('      {0:10},{1:10},{2:15},{3:10},{4:10},{5:20},{6:20},{7:10},{8:10},{9:10},{10:10},{11:10},{12:10}'.format('Amp1','Amp2','P1','Phi1','Phi2','Trend','Quad','Offset','Scale','OffDrop','AmpInc','OffNSop','Lower'))
        for i in self.b_keys:
            print('{0}=[{1:^10.5f},{2:^10.5f},{3:^15.4e},{4:^10.5f},{5:^10.5f},{6:^20.9e},{7:^20.9e},{8:^10.5f},{9:10.5f},{10:10.5f},{11:10.5f},{12:10.5f},{13:10.5}]'.format(i,*self.gdict[i]))
