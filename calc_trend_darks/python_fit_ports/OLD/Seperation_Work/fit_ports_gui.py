...
#set up print, refit, and quit buttons
    ...
    def iris_dark_set(self):
        ...
        for i in ptype: 
            ...
            '''# TIME SEPERATION
            modelA_index = np.where(time < self.freeze_before)
            time = time[modelA_index]
            port = port[modelA_index]
            errs = errs[modelA_index]'''
...
################################################################################################################       
    ...
    def offset(self,dt0,amp1,amp2,p1,phi1,phi2,trend,quad,off,qscale,bo_drop,sc_amp,ns_incr,lower):
       ...
       # Code to find best fit before freeze time
        #self.freeze_before = 1.31e+09
        #post_testy = [(dt0 > self.freeze_before - self.t0dict[self.cport])]
        #pre_testy = [(dt0 < self.freeze_before - self.t0dict[self.cport])]
